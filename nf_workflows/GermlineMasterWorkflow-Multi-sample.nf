...
GenotypeGVCFsExtraOptionsString = params.GenotypeGVCFsExtraOptionsString
...
SampleName = Channel.from(params.SampleName.tokenize(','))	   // Sample name
PlatformUnitChannel = Channel.from(PlatformUnit.tokenize(',')) // Platform Unit
...
process Alignment{
    ...
    output:
        set SampleName, "${SampleName}.${PlatformUnit}.bam", "${SampleName}.${PlatformUnit}.bam.bai" into AlignOutput
...
process Deduplication{
   ...
   input:
       set SampleName, file(InputBams), file(InputBais) from MergeBamsOutput1.groupTuple()  // Link to MergeBams
...
/********      Jointly call: per interval (all samples) - Required    *********/

process JointGenotyping {
   tag "All_samples_${GenomicInterval}"

   input:
        file InputGvcfs from MergeGvcfsOutput.collect() // Link to MergeGvcfs
        file InputIdxs from MergeGvcfsIdx.collect() // Link to MergeGvcfs
        ...
        each GenomicInterval from JointCallIntervals
