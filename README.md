...
Once you have good raw sequences quality, the next step is to map your reads to a reference genome to determine where in the genome the reads originated from. The mapper used in this workflow is BWA.  For more information about the read alignement step, check this [page](https://h3abionet.github.io/H3ABionet-SOPs/Variant-Calling#phase-2-initial-variant-discovery)
...
### 2.4. Variant Calling
This step uses the outputs generated by the Read Alignment STEP! **MUST** run STEP 2.3 (`--mode do.ReadAlignment`) before running this step.
```bash
nextflow run main.nf -profile slurm --mode do.VariantCalling 
```
### 2.5. Variant Filtering
This step uses the outputs generated by the Variant Calling STEP! **MUST** run STEP 2.4 (`--mode do.VariantCalling`) before running this step.
```bash
